...
void main()
{
	...
	{
		InitField(field, n,'0');
		cout << "Сыграем ещё? Да - anykey, Нет - Escape" << endl;
	} while (_getch() != 27);
}
...
void Check(char field[], const int n, char player)
{
	...
	bool draw = true; //изначально предпологаем,что изначально ничья
	for (int i = 0; i < n; i++)
	{
	...
	//if (i == 10)
		//	cout<<player<< "Победила Дружба!\n";
		//		return;

	//for (int i = 0; i <=10; i++)  //сразу выодит ничью
	//{   //if ( i ==10)
	//	cout<<player<< "Победила Дружба!\n";
	//		return;
	//}
